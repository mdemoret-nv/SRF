...
AsyncService::~AsyncService()
{
    if (!m_call_in_destructor_called)
    {
        LOG(ERROR) << this->debug_prefix()
                   << "Must call AsyncService::call_in_destructor to ensure service is cleaned up before being "
                      "destroyed";
    }
}
...
SharedFuture<void> AsyncService::service_start()
{
    ...
    m_completed_future = this->runnable().main().enqueue([this]() {
        ...
        } catch (const std::exception& ex)
        {
            ...
            // Call service kill to initiate the shutdown process
            this->service_kill();
        ...
        // TEMP: Adding an await on live here. This is to ensure that the service is fully started before we start the
        // shutdown process
        this->service_await_live();
...
void AsyncService::service_kill()
{
    ...
    // Ensures this only gets executed once
    if (this->ensure_state(AsyncServiceState::Killing))
    {
        // Kill all children first
        for (auto& child : m_children)
        {
            // Service stop was just called, give them a chance to stop
            auto completed_future = child->completed_future();

            // Only wait() here. Dont care about child errors since we already have an exception
            if (completed_future.wait_for(std::chrono::milliseconds(100)) == boost::fibers::future_status::timeout)
            {
                // Didnt stop in time. Kill the service
                child->service_kill();

                // // Try and wait one more time (just in case)
                // if (completed_future.wait_for(std::chrono::milliseconds(100)) ==
                // boost::fibers::future_status::timeout)
                // {
                //     LOG(ERROR) << this->debug_prefix()
...
void AsyncService::add_state_change_callback(state_change_callback_t callback)
{
    std::unique_lock lock(m_mutex);

    m_state_change_callbacks.push_back(callback);
}
...
void AsyncService::set_parent_service(AsyncService* parent)
{
    std::unique_lock lock(m_mutex);

    m_parent_service = parent;
}
...
{
    {
        ...
        child->add_state_change_callback([this](AsyncServiceState old_state, AsyncServiceState new_state) {
            // If the child is completed, remove it from the list
            if (new_state == AsyncServiceState::Killing)
            {
                // Our child has been killed/died, we should do the same
                this->service_kill();
            }
        });

        child->set_parent_service(this);
...
bool AsyncService::forward_state(AsyncServiceState new_state, bool assert_forward)
{
    ...
    if (m_state < new_state)
    {
        ...
        // Drop the lock before notifying the callbacks to avoid deadlocks
        lock.unlock();

        // Notify any callbacks
        for (const auto& callback : m_state_change_callbacks)
        {
            callback(old_state, m_state);
        }

        if (m_state == AsyncServiceState::Completed)
        {
            // Delete all of the callbacks to avoid circular references
            m_state_change_callbacks.clear();
        }
