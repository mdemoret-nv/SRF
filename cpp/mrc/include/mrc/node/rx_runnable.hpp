...
{
    DVLOG(20) << ctx.info() << " creating composition subscription";
    rxcpp::composite_subscription subscription;
    m_subscription.add(subscription);
    ctx.barrier();
    DVLOG(20) << ctx.info() << " issuing subscribe";
    RxSubscribable::subscribe(subscription);
    DVLOG(20) << ctx.info() << " subscribe completed";
    shutdown(ctx);
    DVLOG(20) << ctx.info() << " shutdown completed";
}
...
void RxRunnable<ContextT>::shutdown(ContextT& ctx)
{
    ...
    if (ctx.rank() == 0)
    {
        DVLOG(20) << ctx.info() << " critical section shutdown - start";
        will_complete();
        on_shutdown_critical_section();
        did_complete();
        DVLOG(20) << ctx.info() << " critical section shutdown - finish";
    }
    ctx.barrier();
}
