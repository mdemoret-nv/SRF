...
{
  public:
    using state_change_callback_t = std::function<void(AsyncServiceState, AsyncServiceState)>;
    ...
    void add_state_change_callback(state_change_callback_t callback);
  ...
  protected:
    ...
    template <typename LaunchableT>
        requires std::derived_from<LaunchableT, runnable::Launchable>
    void child_launchable_start(std::string name, std::shared_ptr<LaunchableT> launchable);

    SharedFuture<void> completed_future() const;

  private:
    void set_parent_service(AsyncService* parent);
...
class AsyncServiceRunnerWrapper : public AsyncService, public runnable::RunnableResourcesProvider
{
  public:
    ...
    AsyncServiceRunnerWrapper(runnable::IRunnableResourcesProvider& resources,
                              std::string name,
                              std::shared_ptr<LaunchableT> launchable) :
      AsyncService(name + "(Launchable)"),
      runnable::RunnableResourcesProvider(resources),
      m_launcher(launchable->prepare_launcher(this->runnable().launch_control()))
    {}

    void kill_on_failure(AsyncService* service)
    {
        m_kill_on_failure = service;
    }
  ...
  private:
    void do_service_start(std::stop_token stop_token) final
    {
        ...
        {
            if (m_kill_on_failure != nullptr)
            {
                m_kill_on_failure->service_kill();
            }
            throw;
        }
    }
    ...
    // If set, kill this service if the runner fails
    AsyncService* m_kill_on_failure{nullptr};
};
...
template <typename LaunchableT>
    requires std::derived_from<LaunchableT, runnable::Launchable>
void AsyncService::child_launchable_start(std::string name, std::shared_ptr<LaunchableT> launchable)
{
    auto wrapper = std::make_unique<AsyncServiceRunnerWrapper>(*this, std::move(name), launchable);
