...
class LaunchControl final
{
  public:
                                                                                  ...
                                                                                  ContextArgsT&&... context_args)
    {
        ...
        {
            CHECK(get_engine_factory(options.engine_factory_name).backend() == EngineType::Fiber) << "Requested "
                                                                                                     "FiberRunnable to "
                                                                                                     "be run on a "
                                                                                                     "ThreadEngine";
  ...
  protected:
    ...
    auto make_contexts(const Runner& runner, const IEngines& engines, ArgsT&&... args)
    {
        ...
        {
            contexts.push_back(
                std::make_shared<WrappedContextT>(resources, runner, i, engines.launchers()[i], args...));
        }
